...
map.addLayer({id: 'locations',
  ...
  map.moveLayer('tram','locations','bhns',);
...
function createPopUp(currentFeature) {
  ...
  const popup = new maplibregl.Popup({ closeOnClick: false })
    ...
    .setHTML(`<h3>${currentFeature.properties.nom}</h3><h4>${currentFeature.properties.capacitesoliste} places théoriques </h4>` + `<h4>${currentFeature.properties.jrdinfosoliste} places disponibles </h4>` + `<h4>${currentFeature.properties.jrdinfoelectrique} borne(s) électrique(s) disponible(s) </h4>`)
    .addTo(map);
}
